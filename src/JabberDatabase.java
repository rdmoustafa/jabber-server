...
public class JabberDatabase {
    ...
    public ArrayList<ArrayList<String>> getMutualFollowUserIDs() {
        ...
        try {
            PreparedStatement stmt = conn.prepareStatement("select f1.userida, f1.useridb from follows f1 " +
                    "inner join follows f2 on f1.userida = f2.useridb and f1.useridb = f2.userida");
    ...
    public ArrayList<ArrayList<String>> getUsersNotFollowed(int userid) {
        ...
        try {
            PreparedStatement stmt = conn.prepareStatement("select username from jabberuser where userid " +
                    "not in (select useridb from follows where userida = ?)");
            stmt.setInt(1, userid);
    ...
    /**
     * This method returns list of [username, jabtext, jabid] for jabs liked by the user.
     * @param userid the userid of the user.
     * @return a list of [username, jabtext, jabid] for jabs liked by the user.
     */
    public ArrayList<ArrayList<String>> getLikesOfUser(int userid) {
        ArrayList<ArrayList<String>> ret = new ArrayList<>();

        try {
            PreparedStatement stmt = conn.prepareStatement("select username, jabtext, jabid from jab " +
                    "natural join jabberuser where jabid in (select jabid from likes where userid = ?)");
    ...
    public ArrayList<ArrayList<String>> getTimelineOfUser(int userid) {
        ...
        try {
            PreparedStatement stmt = conn.prepareStatement("select username, jabtext from jab natural join " +
                    "jabberuser where userid in (select useridb from follows where userida = ?)");
            stmt.setInt(1, userid);
    ...
    public ArrayList<ArrayList<String>> getTimelineOfUserEx(int userid) {
        ...
        try {
            PreparedStatement stmt = conn.prepareStatement("select username, jabtext, jabid, (select " +
                    "count(jabid) from likes A where A.jabid = B.jabid group by A.jabid) as jabcount from jab B " +
                    "natural join jabberuser where userid in (select useridb from follows where userida = ?)");
    ...
    public ArrayList<String> getUsersWithMostFollowers() {
        ...
        String query = "select useridb from follows group by useridb having count (useridb) >= all (select " +
                "count(useridb) from follows group by useridb order by count(useridb));";
    ...
    /**
     * Gets the next available jabID that a new jab should have
     * @return next available jabID
     */
    ...
    /**
     * Gets the next available user ID that a new user signing up can have
    ...
    /**
     * Removes duplicate items from an list of things
     * @param list initial list that may have duplicates
     * @return duplicateless list
     */
    private ArrayList<ArrayList<String>> removeDuplicates(ArrayList<ArrayList<String>> list) {
        ...
        for (ArrayList<String> l: list) { if (!ret.contains(l)) ret.add(l); }
        return ret;
    }
     ...
     */
    private static void print1(ArrayList<String> list) { for (String s: list) { System.out.print(s + " "); } }
    ...
    /**
     * Executes the SQL commands given
